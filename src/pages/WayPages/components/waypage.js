...
export default class WayPage extends React.Component {
    ...
    var table = {
      ...
      onCommandClick: (command, table) => {
        ...
        if (command.command == "Create" || command.command == "Update") {
          this.setState({ tabindex: 1, command: command }, () => {
            if (command.command == "Update") {
              this.editrow = command.selectRows[0]
              this.form.setObj(this.editrow)
            }
          })
          return
        }
  ...
  initForm = () => {
    var form = {
      ...
      onSearch: (value, textbox, callback, form) => {
        this.findDataSource(value, callback)
      }
    }
    return form
  }
        ...
        <RouteContext.Consumer>
          {(value) =>
            (this.state.tabindex == 0 ?
              ...
              : <WayFormTable title={value.title} command={this.state.command} {...form} parent={this} onRef={(ref) => { this.onFormRef(ref) }} ></WayFormTable>
            )
          }
        </RouteContext.Consumer>
      </PageHeaderWrapper >
